...
- **cloud-init drops `<user> ALL=(ALL) NOPASSWD:ALL` for every user it provisions** into `/etc/sudoers.d/90-cloud-init-users`. A new `deploy` user inherits broad NOPASSWD sudo that completely defeats the targeted `command="…"` SSH lockdown — any compromise of the deploy key would have full root. When onboarding a new environment, rewrite that file to remove the deploy user's line right after creating the SSH lockdown, and verify with `sudo -l -U deploy` that the only entry is the bigcapital-specific `(root) NOPASSWD: /srv/portal/clients/<env>/deploy.sh`.
- **`docker compose exec -T <svc> <cmd>` consumes the parent shell's stdin.** Inside a `while read … done <<< "$LIST"` loop, the inner exec eats the rest of the here-string after the first iteration and the loop exits silently. Always pass `< /dev/null` to the exec command when it's inside a stdin-fed loop. Bit `deploy/vps-backup.sh` — only the first DB got dumped.
- **Docker compose project-prefixes network names on disk.** A compose file declaring `bigcapital_sandbox_network` lives at the daemon level as `sandbox-bc_bigcapital_sandbox_network` (project prefix from the dir name). Don't pass the logical name to `docker run --network`; resolve via `docker inspect <container-id> --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}} {{end}}'` and take the first token. `deploy/vps-backup.sh` does this for the transient `minio/mc` mirror container.
- **`mariadb`/`mariadb-dump` only exist in MariaDB 10.4+** — the project pins `mariadb:10.2` (per `docker/mariadb/Dockerfile`), which ships only the legacy `mysql`/`mysqldump`. Backup and admin scripts must use the legacy names for portability across the actually-deployed image versions.
- **`FDateInput` call sites must spread `momentFormatter('MM/DD/YYYY')`** — the upstream `@blueprintjs-formik/datetime` defaults serialize the Formik value via `Date.toISOString()` and parse it via `new Date(str)`, which both cross the local↔UTC boundary and shift the calendar day in any non-UTC client. `momentFormatter` in `packages/webapp/src/utils/index.tsx` overrides `formFormatDate` / `formParseDate` to round-trip as a tz-free `YYYY-MM-DD` string (the parser slices the first 10 chars so server ISO responses survive). A new date field that omits the spread, or passes only a bare `formatDate`/`parseDate` without the form pair, silently re-introduces the day-shift across invoices, money in/out, expenses, etc. Sanity check before merging a new date field: `grep -L 'momentFormatter' <file>` for any file that touches `FDateInput`.
- **Cash-basis financial reports must exclude accrual-only document GL rows AND synthesize from payment events.** `PaymentReceive` / `BillPayment` GL only moves cash ↔ AR/AP — neither touches revenue or expense accounts. Dropping `SaleInvoice` / `Bill` / `CreditNote` / `VendorCredit` rows alone leaves cash-basis revenue stuck at zero. The fork's pattern (see `FinancialStatements/_shared/CashBasisProjection.{service,helpers}.ts` and `ProfitLossSheetRepository.cashBasisAccountsTotal`): when `basis === 'cash'`, run the existing query with `whereNotIn('referenceType', ACCRUAL_ONLY_REFERENCE_TYPES)`, then union with synthetic rows projected from `PaymentReceive`/`BillPayment`/`RefundCreditNote`/`RefundVendorCredit` at the payment date, prorated `payment_amount / document.total` per line. Sibling reports (Balance Sheet, General Ledger, Cash Flow, Sales Tax Liability Summary) accept the flag but still ignore it — pattern lifts onto each report's repository, but each has its own quirks (BS needs the same projection plus zero-AR/AP; SalesTaxLiabilitySummary needs tax-line proration). Adding a new financial report that accepts `basis` without applying this pattern silently makes the toggle a no-op — verify by grep'ing `basis` in the new repo and confirming both the `whereNotIn` filter and the projection union are present.
...
**Open items from the CI/CD bring-up (2026-05-13):**

- **Staging `.env` is missing `WEBAPP_BASE_URL`, `SQUARE_APPLICATION_ID`, `SQUARE_APPLICATION_SECRET`, `SQUARE_APPLICATION_ACCESS_TOKEN`, `SQUARE_ENVIRONMENT`, `SQUARE_OAUTH_REDIRECT_URL`, `SQUARE_TOKEN_ENCRYPTION_KEY`.** Server is running with empty strings; Square integration on staging is silently broken. Fix the `.env` at `/srv/portal/clients/staging-bc/.env` and `docker compose -f docker-compose.ghcr.yml up -d --force-recreate server` to pick up.
- **Trivy gate is warn-only.** Tighten to `exit-code: '1'` after the transitive-dep cleanup: bump `@casl/ability` 5.x→6.x (CRITICAL prototype pollution), dedupe `axios` across `firebase-admin`/`plaid`/etc., move `@babel/plugin-transform-modules-systemjs` and similar dev-tools out of the production install. TODO list in `docs/CI-CD.md`.
- **Restic 0.12.1 on the VPS** lacks compression (0.14+) and parallel prune (0.13+). Backups work fine; upgrade is a perf/dedup-ratio improvement, not a correctness issue.
