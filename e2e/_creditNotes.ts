...
/**
 * Waits until the credit notes list page is loaded.
 */
export async function waitForCreditNotesList(page: Page) {
...
/**
 * Waits until the credit note form page is loaded for the given title
 * (New Credit Note or Edit Credit Note).
 */
export async function waitForCreditNoteForm(page: Page, title: string) {
  await expect(page.getByTestId('dashboard-topbar').locator('h1')).toHaveText(
    title,
    { timeout: 30_000 },
  );
  await expect(page.getByTestId('credit-note-customer-select')).toBeVisible({
    timeout: 30_000,
  });
}

/**
 * Selects the given customer inside the credit note form.
 *
...
export async function selectCustomer(page: Page, displayName: string) {
  const button = page.getByTestId('credit-note-customer-select');
...
/**
 * Creates a credit note through the UI as a draft and expects the redirect
 * back to the credit notes list page. Returns the credit note number the
...
export async function createCreditNote(
  ...
  if (creditNoteNumber) {
    await page
      .getByTestId('credit-note-number-input')
      .fill(creditNoteNumber);
  }

  const createResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname === '/api/credit-notes' &&
  ...
  if (!response.ok()) {
    throw new Error(
      `[e2e] Creating the credit note failed with status ${response.status()}.`,
    );
  }
  ...
  const number = created.creditNoteNumber ?? created.credit_note_number;
  if (!number) {
    throw new Error('[e2e] Creating the credit note returned no number.');
  }
...
/**
 * Filters the credit notes table by the given credit note number and returns
 * the matching row locator. The number is unique per test, so the filtered
 * table holds a single row.
 */
export async function filterCreditNotesByNumber(
  page: Page,
  creditNoteNumber: string,
): Promise<Locator> {
  await page.getByRole('button', { name: /filter|filters applied/i }).click();
  await page.getByPlaceholder('Value').first().fill(creditNoteNumber);

  const row = page
    .getByTestId('credit-note-row')
    .filter({ hasText: creditNoteNumber })
    .first();
...
/**
 * Deletes the given credit note row through the context menu and expects the
 * success toast.
 */
export async function deleteCreditNoteViaRow(page: Page, row: Locator) {
  await row.click({ button: 'right' });
  await page
    .getByRole('menuitem', { name: 'Delete Credit Note' })
    .click();

  await expect(page.getByTestId('credit-note-delete-alert')).toBeVisible();
  await page
    .getByRole('dialog')
    .filter({ has: page.getByTestId('credit-note-delete-alert') })
    .getByRole('button', { name: 'Delete' })
    .click();

  await expect(
    page.getByText('The credit note has been deleted successfully'),
