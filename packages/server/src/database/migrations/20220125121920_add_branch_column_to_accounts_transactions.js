exports.up = (knex) => {
  return knex.schema
    ...
    .table('expenses_transactions', (table) => {
      table.integer('branch_id').unsigned().references('id').inTable('branches').after('user_id');
    })
    ...
    .table('refund_credit_note_transactions', (table) => {
      table.integer('branch_id').unsigned().references('id').inTable('branches').after('description');
    })
