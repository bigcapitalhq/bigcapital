...
export class WarehousesDTOValidators {
   ...
   */
  public validateDTOWarehouseExistance = async (tenantId: number, DTO: IWarehouseTransactionDTO) => {
    // Validates the sale invoice warehouse id existance.
    this.validateWarehouseExistanceService.validateWarehouseIdExistance(DTO, DTO.entries);
    // Validate the sale invoice warehouse existance on the storage.
    if (DTO.warehouseId) {
      this.validateWarehouseExistanceService.validateWarehouseExistance(tenantId, DTO.warehouseId);
    }
    // Validate the sale invoice entries warehouses existance on the storage.
    if (!isEmpty(DTO.entries)) {
      await this.validateWarehouseExistanceService.validateItemEntriesWarehousesExistance(tenantId, DTO.entries);
    }
  };
   ...
   * @returns
   */
  public validateDTOWarehouseWhenActive = async (tenantId: number, DTO: IWarehouseTransactionDTO): Promise<void> => {
