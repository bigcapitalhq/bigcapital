...
export class ImportFileCommon {
  ...
  public async import(
    ...
    const importableFields = this.resource.getResourceImportableFields(tenantId, importFile.resource);
    ...
    const importAsync = async (objectDTO, index: number): Promise<void> => {
      ...
      try {
        ...
        await this.importFileValidator.validateData(importableFields, transformedDTO);
        try {
