...
import { getDefaultPLQuery } from './utils';

describe('ProfitLossSheetQuery', () => {
  const baseQuery = () => getDefaultPLQuery();

  it('enables the amount change and previous period when the percentage change is on', () => {
    const query = new ProfitLossSheetQuery({
      ...baseQuery(),
  ...
  it('keeps the comparison flags disabled by default', () => {
    const query = new ProfitLossSheetQuery(baseQuery());
