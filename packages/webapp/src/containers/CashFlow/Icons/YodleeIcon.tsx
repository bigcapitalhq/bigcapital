export const YodleeIcon = (props: any) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlSpace="preserve"
    id="Layer_1"
    x={0}
    y={0}
    style={{
      width: 28,
      height: 28,
    }}
    viewBox="0 0 30 57.5"
    {...props}
  >
    <style>{'.st0{fill:#343741}.st1{fill:#0084d4}.st3{fill:#8bd3e6}'}</style>
    <path
      d="m15.1 30.2 4.4-1.4-4.4-1.4-4.4 1.4z"
      style={{
        fill: 'none',
      }}
    />
    <path
      d="M0 0v9.8l19.5 6.3 15-4.9zM0 12.7v9.8l15.1-4.9zM19.5 28.8l15-4.9-4.3-1.4-15.1 4.9zM19.5 41.4l15-4.9-4.3-1.4L15.1 40zM10.7 28.8 0 32.3v2.8l15.1-4.9z"
      className="st3"
    />
    <path d="M0 35.1v9.8L15.1 40zM15.1 30.2l15.1 4.9 4.3-1.4-15-4.9z" className="st0" />
    <path d="M34.5 33.7v-9.8l-15 4.9zM0 35.1 15.1 40l15.1-4.9-15.1-4.9z" className="st1" />
    <path d="M0 47.7v9.8l34.5-11.2-15-4.9z" className="st0" />
    <path d="M34.5 46.3v-9.8l-15 4.9z" className="st1" />
    <path d="m15.1 17.6 15.1 4.9 4.3-1.5-15-4.9zM15.1 27.4 0 22.5v2.8l10.7 3.5z" className="st0" />
    <path d="M34.5 21v-9.8l-15 4.9zM0 22.5l15.1 4.9 15.1-4.9-15.1-4.9zM0 25.3v7l10.7-3.5z" className="st1" />
  </svg>
);
