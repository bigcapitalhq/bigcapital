...
export type SettingsResponse = OpResponseBody<
  OpForPath<typeof SETTINGS_ROUTES.GET_SAVE, "get">
>;
export type SaveSettingsBody = OpRequestBody<
  OpForPath<typeof SETTINGS_ROUTES.GET_SAVE, "put">
>;
export type GetSettingsQuery = OpQueryParams<
  OpForPath<typeof SETTINGS_ROUTES.GET_SAVE, "get">
>;
 ...
 */
export function transformSettingsResponse(
...
export async function fetchSettings(
  ...
  const { data } = await (
    get as (params?: GetSettingsQuery) => Promise<{ data: SettingsResponse }>
  )(query ?? {});
  return data;
}
 ...
 */
export async function fetchSettingsGrouped(
  fetcher: ApiFetcher,
): Promise<AllSettings> {
...
export const SMS_NOTIFICATIONS_ROUTES = {
  LIST: "/api/settings/sms-notifications",
  BY_KEY: "/api/settings/sms-notification/{key}",
  EDIT: "/api/settings/sms-notification",
} as const satisfies Record<string, keyof paths>;

export type SmsNotificationSetting = OpResponseBody<
  OpForPath<typeof SMS_NOTIFICATIONS_ROUTES.BY_KEY, "get">
>;
export type SmsNotificationSettingsListResponse = OpResponseBody<
  OpForPath<typeof SMS_NOTIFICATIONS_ROUTES.LIST, "get">
>;
export type EditSmsNotificationBody = OpRequestBody<
  OpForPath<typeof SMS_NOTIFICATIONS_ROUTES.EDIT, "post">
>;
export type EditSmsNotificationValues = Omit<EditSmsNotificationBody, "key">;

export async function fetchSettingSMSNotifications(
  fetcher: ApiFetcher,
): Promise<SmsNotificationSettingsListResponse> {
  const get = fetcher
    .path(SMS_NOTIFICATIONS_ROUTES.LIST)
    .method("get")
    .create();
  const { data } = await (
    get as (
      params?: Record<string, never>,
    ) => Promise<{ data: SmsNotificationSettingsListResponse }>
  )({});
  return data;
}
...
export async function fetchSettingSMSNotification(
  fetcher: ApiFetcher,
  key: string,
): Promise<SmsNotificationSetting> {
  const get = fetcher
    .path(SMS_NOTIFICATIONS_ROUTES.BY_KEY)
    .method("get")
    .create();
  const { data } = await (
    get as (params: {
      key: string;
    }) => Promise<{ data: SmsNotificationSetting }>
  )({ key });
  return data;
}
...
export async function editSettingSMSNotification(
  ...
  const post = fetcher
    .path(SMS_NOTIFICATIONS_ROUTES.EDIT)
    .method("post")
    .create();
  const { data } = await (
    post as (
      params: EditSmsNotificationBody,
    ) => Promise<{ data: SmsNotificationSetting }>
  )({ key, ...values });
  return data;
}
